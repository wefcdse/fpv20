...
//        this.angular_speed = angular_speed;
        ...
        this.droneRotation = new Quaternion(0, 0, 0, false);
        this.lastDroneRotation = new Quaternion(0, 0, 0, false);
//        this.cam_angel_deg = 30;
    ...
    public static void setFlying(boolean if_fly) {
        ...
        if (p != null) {
            ...
            if (ClientPlayNetworking.canSend(DroneFlyPacket.ID)) {
                ClientPlayNetworking.send(DroneFlyPacket.ID, new DroneFlyPacket(if_fly).asPacket());
            }
    ...
    public Quaternion cacl_cam_rotation() {
        Quaternion new_r = new Quaternion(this.droneRotation);

//        new_r.rotateLocalX(-Fpv20Client.config1.getCamera_angle() * DEG_TO_RAD);
        PortMath.rotateLocalX(new_r, -Fpv20Client.config1.getCamera_angle() * DEG_TO_RAD, new_r);

        return new_r;
    }

    @Deprecated
    public Quaternion cacl_cam_rotation_last() {
        Quaternion new_r = new Quaternion(this.lastDroneRotation);
...
//    }
    ...
    private void set_drone_rotation(Quaternion camPos) {
        this.lastDroneRotation = this.droneRotation;
        this.droneRotation = new Quaternion(camPos);
    }
...
//        float roll = this.camRoll;
        ...
        Vec3f vd = new Vec3f((float) v.x, (float) v.y, (float) v.z);
        Vec3d pos = p.getPos();
...
//        p.setVelocity(drone.get_speed());
        ...
        Vec3f new_ypr = PhysicsCore.from_quaternion_to_ypr_deg(this.cacl_cam_rotation());
        if (false) {
//            p.setYaw(new_ypr.x);
...
//            if (vd.length() > aaa * dt && vd.length() > 0.0000001) {
            if (PortVec3f.length(vd) > aaa * dt && PortVec3f.length(vd) > 0.0000001) {
//                Vector3f vd1 = new Vector3f(vd).normalize().mul(-1f * aaa * dt);
                Vec3f vd1 = new Vec3f(vd.getX(), vd.getY(), vd.getZ());
                vd1.normalize();
                vd1.scale(-1f * aaa * dt);
                vd.add(vd1);
            } else {
//                vd.zero();
    ...
    public void handle_flying_rotate(MinecraftClient client, float dt) {
        ...
        if (Fpv20.config.in_fabric()) {
            if (!Fpv20Client.config1.free_camera_yaw) {
                p.setYaw(new_ypr.getX());
            }
            if (!Fpv20Client.config1.free_camera_pitch) {
                p.setPitch(new_ypr.getY());
            }
        } else {
            if (!Fpv20Client.config1.free_camera_yaw) {
                p.setYaw(180);
