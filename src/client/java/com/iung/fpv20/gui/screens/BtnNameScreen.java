...
public class BtnNameScreen extends BackableScreen {
    int channel;

    protected BtnNameScreen(Screen parent, int channel) {
        super(Text.literal("btn_name"), parent);
...
//        }).dimensions(slide_start, 30 + (padding + height) * 2, this.width - slide_start - padding, height).build());
        ...
        this.addDrawable(new Value01Display(i + (width - 20) / 2, 30 + padding + height, 20, height2, () -> {
            Controller ctrl = Fpv20Client.controller;
            if (ctrl != null) {
                return ctrl.get_btn(channel);
            } else {
                return 0;
            }
        }));
...
//        }));
        ...
        String text = controller1.get_btn_name(channel);
        this.addDrawableChild(new TextWidget(
                j, 30 + (padding + height) * 2, width, height,
...
//        this.addDrawable(new Input);
        TextFieldWidget tfw = this.addDrawableChild(new TextFieldWidget(this.textRenderer,
                j, 30 + (padding + height) * 3, width, height,
...
//            Fpv20.LOGGER.info("{}", str);
            Controller controller = Fpv20Client.controller;
            if (controller != null) {
                controller.set_btn_name(channel, str);
            }
        });
    }
